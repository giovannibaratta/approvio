...
## Security registration for new tables

The database bootstrap installs a PostgreSQL event trigger that requires every future public table to be registered
before creation. A table migration must insert its security class and allowlisted capability profile first:
...
- createTable:
    tableName: workflow_attempts
```

The subsequent `CREATE TABLE` causes the event trigger to validate the table shape and apply the profile. Tenant
tables receive forced row-level security and a policy that compares each row's `organization_id` with the
transaction-local PostgreSQL setting `approvio.organization_id`. The policy uses that comparison for both reads and
writes: `USING` limits which existing rows are visible or can be changed, while `WITH CHECK` rejects inserted or
updated rows belonging to another organization. If the setting is missing, the comparison does not match, so the
policy grants no row access. Capability grants are applied only to roles in the selected profile. Untagged tables
fail creation. There are no broad default table grants for runtime roles. The registry and profile tables are
internal security metadata and are not granted to runtime roles.

The event trigger is deliberately PostgreSQL-specific. The acceptance test also discovers public tables from the
catalog and verifies that every table is registered and active, and that every non-null `organization_id` table has
both RLS flags enabled.

Capability profiles are allowlisted bundles of table privileges for runtime roles; table migrations select a profile
but cannot define new roles or grants.
