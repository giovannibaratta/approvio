...
export class AgentFactory {
  ...
  static validate<T extends AgentDecoratorSelector>(
    data: Omit<DecoratedAgent<T>, "roles" | "status"> & {readonly roles: unknown; readonly status: unknown}
    ...
    return pipe(
      ...
      E.bindW("validatedStatus", () => this.validateStatus(data.status)),
      E.bindW(
        "baseObj",
        ({validatedId, validatedOrganizationId, validatedAgentName, validatedRoles, validatedStatus}) => {
  ...
  private static validateStatus(status: unknown): Either<AgentValidationError, Agent["status"]> {
    return status === "active" || status === "revoked" ? E.right(status) : E.left("agent_invalid_status")
  }
