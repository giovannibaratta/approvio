...
import {Brand, brand, isUUIDv7} from "@utils"
...
declare const TenantPrincipalBrand: unique symbol
 ...
 */
export type TenantPrincipal = Brand<TenantPrincipalData, typeof TenantPrincipalBrand>
...
// TODO: TenantPricinpal is not used anywhere ? Will it be reference at some point ?
export class TenantPrincipalFactory {
  ...
  static fromVerified(input: {
    ...
    if (!isUUIDv7(input.organizationId)) {
      return left("principal_invalid_organization_id")
    }

    if (input.credential.kind === "tenant-user") {
      if (input.credential.organizationId !== input.organizationId) {
        return left("principal_organization_mismatch")
      }
    ...
    } else if (input.credential.kind === "platform-user") {
      return left("principal_invalid_credential")
    }
    ...
    return right(brand<TenantPrincipalData, typeof TenantPrincipalBrand>(data))
  }
}
