...
export class UserFactory {
   ...
   * @param roleAssignments Array data that should represent BoundRole array
   * @returns Either validation error or validated roles array
   */
  static validateRoleAssignments(
    roleAssignments: unknown
  ): Either<UserValidationError, ReadonlyArray<BoundRole<string>>> {
    if (roleAssignments === null || roleAssignments === undefined) return right([])
    if (!Array.isArray(roleAssignments)) return left("user_role_assignments_invalid_format")

    return RoleFactory.validateBoundRoles(roleAssignments)
  }
  ...
  private static createUser(
    ...
    const rolesValidation = this.validateRoleAssignments(data.roles)
