...
type GroupWithMemberships = PrismaGroup & {groupMemberships: (PrismaGroupMembership & {users: PrismaUser})[]}
...
export class GroupMembershipDbRepository implements GroupMembershipRepository {
  ...
  private getUserMembershipsByUserIdTask(): (
    userId: string
  ) => TaskEither<UnknownError, ReadonlyArray<PrismaGroupMembership & {users: PrismaUser}>> {
    return userId =>
      TE.tryCatchK(
        () =>
...
function mapMembershipToDomain(
  dbObject: PrismaGroupMembership & {users: PrismaUser}
...
function mapToDomainMembershipWithGroupRefs(
  dbObject: ReadonlyArray<PrismaGroupMembership & {users: PrismaUser}>
...
function mapToDomainMembershipWithGroupRef(
  dbObject: Readonly<PrismaGroupMembership & {users: PrismaUser}>
