...
export class OidcBootstrapService implements OnApplicationBootstrap {
  ...
  getConfiguration(providerId: string): Either<OidcError, OidcServerMetadata> {
    const config = this.validatedConfigurations.get(providerId)
    if (!config) {
      this.logger.error(`OIDC configuration not found or not initialized for provider ${providerId}`)
      return E.left("oidc_provider_not_found" as const)
    }

    return E.right(config)
  }

  getRawClientConfiguration(providerId: string): Either<OidcError, client.Configuration> {
    const config = this.rawConfigurations.get(providerId)
    if (!config) {
      this.logger.error(`OIDC raw configuration not found or not initialized for provider ${providerId}`)
      return E.left("oidc_provider_not_found" as const)
    }
