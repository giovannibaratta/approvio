...
/**
 * Validated UserInfo response conforming to OpenID Connect specification.
 * The 'sub' claim is guaranteed to be present as required by the RFC.
 */
...
export function validateUserInfoResponse(
...
): E.Either<UserInfoValidationError, ValidatedUserInfo> {
  ...
  const validatedUserInfo: ValidatedUserInfo = {
    sub: rawResponse.sub
  }

  // Add optional claims if present and valid - using object construction
  const result: ValidatedUserInfo = {
    ...validatedUserInfo,
