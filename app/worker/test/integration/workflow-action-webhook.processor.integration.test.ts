...
import {setupSimpleStub, getWiremockRequests, resetWiremock, getWiremockBaseUrl} from "./wiremock-utils"
...
describe("Workflow Action Webhook Processor Integration", () => {
  ...
  describe("process", () => {
    // Wiremock base URL (dockerized Wiremock runs on port 9090)
    const WIREMOCK_BASE_URL = getWiremockBaseUrl()
    ...
    it("should successfully process a webhook task and update task status to COMPLETED", async () => {
      ...
      const webhookUrl = `${WIREMOCK_BASE_URL}${uniqueWebhookPath}`
      const {taskId, workflowId} = await createWorkflowWithWebhookTask(prisma, webhookUrl)

      // Configure Wiremock to respond to the webhook call
      await setupSimpleStub("POST", uniqueWebhookPath, 200, {success: true, message: "Webhook received"})
      ...
      const wiremockRequests = await getWiremockRequests()
      const webhookRequests = wiremockRequests.filter(req => req.url === uniqueWebhookPath)
    ...
    it("should handle webhook failures and update task status to ERROR", async () => {
      ...
      const failingWebhookPath = `/failing-webhook-${randomUUID()}`
      const webhookUrl = `${WIREMOCK_BASE_URL}${failingWebhookPath}`
      const {taskId, workflowId} = await createWorkflowWithWebhookTask(prisma, webhookUrl)

      // Configure Wiremock to respond with an error
      await setupSimpleStub("POST", failingWebhookPath, 500, {error: "Internal Server Error"})
      ...
      // Expect: The webhook was called
      const wiremockRequests = await getWiremockRequests()
      const webhookRequests = wiremockRequests.filter(req => req.url === failingWebhookPath)
      expect(webhookRequests).toHaveLength(1)

      // And: The task was updated to ERROR status due to 500 response
      const updatedTask = await prisma.workflowActionsWebhookTask.findUnique({
        where: {id: taskId}
      })
