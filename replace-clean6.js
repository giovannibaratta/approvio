  ...
  while (i < lines.length) {
    ...
    if (trimmed.startsWith('case "')) {
      ...
      if (j < lines.length && lines[j] !== undefined && lines[j].includes('Logger.error(`Internal data inconsistency: ${errorCode}`)')) {
        ...
        // consume trailing blank lines inside the block
        // Actually do not consume them here, or if we do, just be careful
        i = j;
        continue;
      }
    }
    ...
    if (trimmed.startsWith('switch (error) {') || trimmed.startsWith('switch(error) {') || trimmed === 'switch (error) {') {
