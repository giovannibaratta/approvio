...
const decision = await featureGateService.isFeatureEnabled(FeatureKey.PLATFORM_LLM_EVALUATORS, {orgId, actorId})
```
...
1. **Redis (Fast Admission & Reservation):** Tracks active billing period consumption and holds pending reservations. Answers: _"Can this tenant execute right now?"_
2. **PostgreSQL (Durable Usage Ledger):** Stores immutable `usage_events` records. Answers: _"What did this tenant consume, when, and by which actor?"_ Used for invoice reconciliation, usage dashboards, billing exports, and dispute audits.
...
2. **Call-Based Metering (`MAX_EVALUATIONS_PER_MONTH`):**
   ...
   - **Trade-offs:** Highly intuitive to market (e.g., _"1,000 evaluations/month"_). However, it exposes the platform to financial risk if prompt payloads or reasoning steps vary widely.
...
- **Dangling Reservation Risk:** Instance termination during an inflight LLM call could leave reserved capacity held.
  - _Mitigation:_ Inflight reservations include expiration TTLs and are reconciled by background health sweeps.
